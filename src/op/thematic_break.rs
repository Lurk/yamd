...
fn is_dash(t: &Token) -> bool {
    t.kind == TokenKind::Minus && t.position.is_line_start && t.range.len() == 5
}
...
pub fn thematic_break(p: &mut Parser) -> bool {
    ...
    p.ops
        .push(Op::new_start(Node::ThematicBreak, Content::empty()));
...
mod tests {
    ...
    fn happy_path() {
        ...
        assert_eq!(
            p.ops,
            vec![
                Op::new_start(Node::ThematicBreak, Content::empty()),
                Op::new_value(p.span(0..1)),
                Op::new_end(Node::ThematicBreak, Content::empty()),
            ]
        );
    }
